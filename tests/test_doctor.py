...
class TestDoctor:
    def _make_env(self, tmp_path, has_skills=True, has_mounts=True, mounts_ok=True, has_mcp=True, mcp_has_reckon=True):
        ...
        if has_skills:
            for skill in ["reckon-create", "reckon-edit", "reckon-implement", "reckon-sprint", "reckon-status", "reckon-sync"]:
        ...
        if has_mcp:
            ...
            cfg = {"mcpServers": {}}
            if mcp_has_reckon:
                cfg["mcpServers"]["reckon"] = {"command": "uv", "args": ["run", "reckon", "mcp"]}
    ...
    def test_happy_path(self, tmp_path):
        result = self._run_doctor(tmp_path,
            has_skills=True, has_mounts=True, mounts_ok=True,
            has_mcp=True, mcp_has_reckon=True)
        assert result.exit_code == 0
        assert "All checks passed" in result.output

    def test_missing_skill(self, tmp_path):
        result = self._run_doctor(tmp_path,
            has_skills=False, has_mounts=True, mounts_ok=True,
            has_mcp=True, mcp_has_reckon=True)
        assert result.exit_code != 0
    ...
    def test_missing_mount_dir(self, tmp_path):
        result = self._run_doctor(tmp_path,
            has_skills=True, has_mounts=True, mounts_ok=False,
            has_mcp=True, mcp_has_reckon=True)
        assert result.exit_code != 0
    ...
    def test_no_mounts_file(self, tmp_path):
        result = self._run_doctor(tmp_path,
            has_skills=True, has_mounts=False, mounts_ok=True,
            has_mcp=True, mcp_has_reckon=True)
        assert result.exit_code != 0
