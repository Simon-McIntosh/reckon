...
The document is the observer's one reconciled record of every metered account,
and the production callers -- ``reckon crew preflight`` and the MCP ``budget``
view -- always inject a window reading of their own from what earlier runs
recorded. That reading used to be the whole answer: the document was read only
when a caller named it *and* injected nothing, so the two paths that actually
run never heard the published document at all. These cases pin the merge that
fixes it: a backend the document covers freshly is paced from it, and every
other backend keeps the caller's recorded reading, with the report naming which
source spoke for each.

The temporary home matters. ``paid_lanes.document_path()`` resolves the
published document under the reader's home, so a case that pointed at the real
one would report on the workstation rather than on its fixture. Each case
installs a home inside the temporary directory, and the default path the
production callers resolve lands there.
"""
...
def _install_default_document(
    ...
    """Install the document at the default path under a temporary home.

    ``paid_lanes.document_path()`` resolves the environment override, then the
    reader's home. The override is cleared and ``HOME`` is pointed at the
    temporary tree, so the path the production callers resolve is the fixture's
    own and never the workstation's published document.
    """
    monkeypatch.delenv(paid_lanes.DOCUMENT_ENV, raising=False)
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    path = home / "public" / "reckon" / "paid-lanes.json"
    paid_lanes.write_document_atomically(document, path)
    # The default path the callers name is this file, proven rather than assumed.
    assert paid_lanes.document_path() == path
    return path
