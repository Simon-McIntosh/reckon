...
def _make_plan_html(
    ...
    *,
    ...
    typed_state = {**state, "type": state.get("type", artifact_type)}
    html_with_island = write_state(bare, typed_state)
...
def test_write_plan_roundtrip(setup):
    ...
    docs_dir, _, project = setup
    _make_plan_html(
        docs_dir, "plan-a", {"slug": "plan-a", "status": "draft", "version": 0}
    )

    initial_data = {"slug": "plan-a", "status": "active", "impl": 0.5}
    new_version = _store_module.write_plan(
        project, "plan-a", initial_data, expected_version=0
    )
...
def test_write_plan_requires_type_for_duplicate_leaf(setup):
    ...
    _make_plan_html(
        docs_dir,
        "shared",
        {"slug": "shared", "version": 0},
        relative="plans/shared.html",
    )
    research = _make_plan_html(
        docs_dir,
        "shared",
        {"slug": "shared", "version": 4, "status": ""},
        artifact_type="research",
        relative="research/shared.html",
    )

    with pytest.raises(ValueError, match="ambiguous across types"):
        _store_module.write_plan(
            project,
            "shared",
            {"slug": "shared", "type": "plan"},
            expected_version=0,
        )
...
def test_write_plan_rejects_selected_type_mismatch(setup):
    docs_dir, _, project = setup
    research = _make_plan_html(
        docs_dir,
        "shared",
        {"slug": "shared", "version": 2, "status": ""},
        artifact_type="research",
        relative="research/shared.html",
    )
    before = research.read_bytes()

    with pytest.raises(ValueError, match="does not match selected resource type"):
        _store_module.write_plan(
            project,
            "shared",
            {"slug": "shared", "type": "evidence", "version": 2},
            expected_version=2,
            artifact_type="research",
        )
...
def test_write_plan_modified_date_set(setup):
    ...
    _store_module.write_plan(
        project, "plan-b", {"status": "active"}, expected_version=0
    )
...
def test_write_plan_atomic_tmp_rename(setup):
    ...
    _store_module.write_plan(
        project, "plan-c", {"status": "active"}, expected_version=0
    )
...
def test_version_conflict_carries_current_data(setup):
    ...
    _store_module.write_plan(
        project, "plan-e", {"status": "active"}, expected_version=0
    )
    try:
        _store_module.write_plan(
            project, "plan-e", {"status": "blocked"}, expected_version=0
        )
...
def test_append_followup(setup):
    ...
    _store_module.append_to_list(
        project, "plan-h", "followups", followup2, expected_version=1
    )
    data, _ = _store_module.read_plan(project, "plan-h")
...
def test_resolve_in_list_missing_id_raises(setup):
    ...
    with pytest.raises(KeyError):
        _store_module.resolve_in_list(
            project, "plan-j", "followups", "NOPE", {"outcome": "x"}, expected_version=0
        )
