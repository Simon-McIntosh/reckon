...
def test_the_pace_multiple_is_a_configured_key_that_moves_the_allowance():
    base = allowance_for(STATES[ON_PACE])
    doubled = allowance_for(
        STATES[ON_PACE],
        {"budget": {"drain_lead_hours": LEAD_HOURS, "pace_multiple": 2.2}},
    )
...
def test_the_drain_lead_is_a_configured_key_that_moves_the_allowance():
    base = allowance_for(STATES[ON_PACE])
    early = allowance_for(
        STATES[ON_PACE],
        {"budget": {"drain_lead_hours": 24.0, "pace_multiple": MULTIPLE}},
    )
...
def test_neither_tunable_stands_in_for_the_other():
    base = allowance_for(STATES[ON_PACE])
    lead_moved = allowance_for(
        STATES[ON_PACE],
        {"budget": {"drain_lead_hours": 30.0, "pace_multiple": MULTIPLE}},
    )
    multiple_moved = allowance_for(
        STATES[ON_PACE],
        {"budget": {"drain_lead_hours": LEAD_HOURS, "pace_multiple": 3.3}},
    )
...
def test_every_allowance_carries_the_multiple_that_produced_it(state):
