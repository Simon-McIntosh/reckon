...
Two runs cross the same stall window with the same quiet time and opposite
remedies: one whose worker process is still alive and thinking, and one whose
process is gone. The row names which is which, in words, beside the minutes it
has been quiet — so the distinction reaches a reader who cannot see a colour,
and a coordinator acts on the reading rather than on the word "stalled" alone.
...
def main() -> None:
    fig, (timeline, clause) = plt.subplots(
        2, 1, figsize=(11.0, 6.2), gridspec_kw={"height_ratios": [1.0, 1.15]}
    )
    fig.suptitle(
    ...
    rows = [
        (1.0, "worker process alive", LIVE_COLOUR),
    ...
    steps = [
        ...
        (
            ...
            "True  →  alive          False, None  →  process gone",
        ),
