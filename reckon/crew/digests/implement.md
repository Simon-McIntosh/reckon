...
#
# Sections retained, in canonical order (22):
#   ## Git Safety
#   ### Banned Commands
...
A login node carries one user's whole fleet under one uid: peer agents, zellij
servers holding days of session state, the systemd user manager, and the single
SSH connection every one of them is reached through. A kill aimed by pattern
cannot see any of that, so it takes whatever the pattern happens to match.

- **Never kill by pattern — resolve to pids, print them, then signal that
  list.** `pkill -f`, `killall`, and `pgrep … | xargs kill` all act on a set
  the author never saw. Walk the tree to a fixed point (`ps --ppid`), show the
  members with their ages and command lines, and signal only those pids.
  SIGTERM first; escalate to SIGKILL only for what is still alive afterwards.
- **`pkill -f` matches the command line of the shell running it.** Measured
  2026-09-18 on a login node: `ssh <host> 'pkill -f "attach --create probe";
  …'` matched the remote `bash -c` whose own argv contained that exact string,
  killed its own shell mid-script, returned 255, and left the rest of the
  cleanup unrun. A pattern passed through ssh, `bash -c`, or a tool call is by
  construction present in the target's argv. Where a pattern is unavoidable,
  bracket a character to break the self-match (`probe[-]detach`) and exclude
  `$$`.
- **A term signal to a zellij server is not a stop, and the disguise is the
  danger.** Under `on_force_close "detach"` the server answers by detaching
  its clients, so a stray kill does not present as a kill — it presents as the
  connection having dropped, and the investigation goes to the network. Reap a
  zellij server only on a host-local, unambiguous fact: its socket is gone from
  `/run/user/<uid>/zellij/<contract>/`. Never on "the current binary cannot see
  it", which is equally true of every session alive on another login node.
- **Never restart or stop `user@<uid>.service`, and never `loginctl
  terminate-user`.** That unit owns the lingering services; cycling it stops
  everything beneath it and recreates the session bus under every process still
  holding a handle to it.
- **Anything selected through the shared home is cross-node.** `~/.cache` and
  `~/.config` are the same bytes on every login node while `/run/user` is not,
  so a session alive on another node reads locally as dead. Before deleting or
  killing anything chosen from a shared path, prove liveness from a file the
  live process keeps rewriting — compare its mtime against now — rather than
  from the absence of a local socket.
- **State the blast radius before signalling.** Name the peer agents, zellij
  servers and services inside it and say why each survivor is outside it. A
  kill whose radius was never written down was never bounded.
## Bug and Failure Ownership
...
Model family, concrete model, reasoning effort, and worker concurrency are
...
- Pipeline model seats configured by an application are separate product
  configuration; they do not control agent or sub-agent routing.
## Compute Infrastructure (SDCC) — never heavy work on the login node (binding)
