...
Reading the first turn's own usage record rather than the run's terminal
cumulative total is the point of the measure. The cumulative total re-counts the
same history on every turn, so it describes the whole run's spend rather than
what was read; a run with many cheap turns reports a figure far larger than any
single request, and a fresh run whose turns are many reads as if it had
inherited a great deal. The two fields are not interchangeable and the census
uses the first-turn one.
...
* ``claude`` — the claude-code harness writes ``assistant`` records whose
  ...
  whose ``modelUsage.inputTokens`` is the run's session-cumulative charged input
  (every turn's full prompt, cache reads included). The census reads the first
  ``assistant`` record.
...
def _first_turn_input(stream: Path, dialect: str | None) -> float | None:
    ...
    its modelUsage is the session-cumulative total, which is a different
    quantity.
    """
...
def _run_cumulative_input(stream: Path, dialect: str | None) -> float | None:
    """Read a run's terminal cumulative charged input, per dialect.
    ...
    is the ``result`` record's ``modelUsage`` input for the run's model.  Recorded
    as a distinct field so a reader sees what was actually published rather than
    mistaking the sum for the first turn.
    """
...
def carryover_census(
    ...
    *,
    ...
    for dialect in sorted({row["dialect"] for row in rows}):
        ...
        baselines[dialect] = _median(
            row["first_turn_input_tokens"] for row in opening
        )
    ...
    for row in rows:
        ...
        row["backend_basis"] = (
            "recorded"
            if recorded_backends.get(row["run_id"])
            else "dialect-lane"
        )
        ...
        if row.get("resumed") is False:
            ...
            row["inherited_basis"] = (
                "first-turn-minus-own-prompt-and-harness-preamble"
            )
    ...
    for lane in sorted({str(row["backend"]) for row in rows}):
        ...
        over_own = [
            row
            for row in resumed
            if row["inherited_over_own_prompt"] is True
        ]
        per_backend[lane] = {
            "runs": len(lane_rows),
...
def _result_model_usage_input(stream: Path) -> dict[str, Any] | None:
    """Read a run's terminal modelUsage input, the cumulative-total example.
