...
MANIFEST (write exactly these keys; after reading the plan, observe path and revision in the assigned tree and make these first three lines your first write)
  ...
  checkpoint: <only when not setting status to waiting: one line recording where work stands and what comes next, so a resumed worker whose wait is met leaves a checkpoint rather than a wait block>
  commits: <sha list>
